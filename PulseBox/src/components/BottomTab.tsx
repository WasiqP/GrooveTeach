...
import { theme, ink } from '../theme';

/** Space between icons and the top of the system nav / home indicator zone */
const TAB_CONTENT_BOTTOM = 12;

const TAB_INACTIVE = 'rgba(26, 26, 34, 0.45)';

// SVG Icons — GrooveBox
const HomeIcon = ({ size = 24, color = theme.primary }) => (
...
// Class/Book Icon for My Classes
const ClassIcon = ({ size = 24, color = theme.primary }) => (
  <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
...
// Quiz/Assignment Icon
const QuizIcon = ({ size = 24, color = theme.primary }) => (
  <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
...
const SettingsIcon = ({ size = 24, color = theme.primary }) => (
  <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
...
const BottomTab: React.FC<BottomTabProps> = ({ navigation, currentRoute, useSafeArea = true }) => {
  ...
  const paddingBottom = useSafeArea
    ? TAB_CONTENT_BOTTOM + insets.bottom
    : 16;
  ...
  const getIconColor = (routeName: string) => {
    return currentRoute === routeName ? theme.primary : TAB_INACTIVE;
  };
...
const styles = StyleSheet.create({
  bottomNav: {
    ...
    paddingTop: 20,
    // paddingBottom set in component: TAB_CONTENT_BOTTOM + insets.bottom
